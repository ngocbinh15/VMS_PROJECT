...
INSERT INTO DmDonVi
  (Id, MaDonVi, TenDonVi, LoaiDonVi, ThuTuHienThi, NguoiTao)
VALUES
...
INSERT INTO DmTramQuanLy
  (Id, MaTram, TenTram, DonViChuQuanId, ThuTuHienThi, NguoiTao)
VALUES
  (1, N'TQBHLHH_QN', N'Trạm QL báo hiệu luồng Quy Nhơn', 1, 1, N'SystemAdmin');
...
BEGIN
  ...
  VALUES
    (N'TUYEN_' + CAST(@t AS NVARCHAR), N'Tuyến luồng số ' + CAST(@t AS NVARCHAR), @t, N'SystemAdmin');
  SET @t = @t + 1;
END;
...
BEGIN
  ...
  INSERT INTO DmViTriPhaoBH
    (TuyenLuongId, SoViTri, MaPhaoBH, ToaDoThietKe, MoTa, ThuTuHienThi, NguoiTao)
  VALUES
    (
      @TuyenId,
      CAST(@v AS NVARCHAR),
...
BEGIN
  ...
  DECLARE @CurrentViTri INT = @PhaoId;
  -- Mỗi phao lấy 1 vị trí ban đầu (1 đến 141)
